<?php
    ...
    // UPGRADE TO V2.8.11
    if ($common->getSetting("version") == "2.8.10" && $success) {
        $json = file_get_contents("http://localhost/install/upgrade-v2.8.11.php");
        $results = json_decode($json, TRUE);
        $success = $results['success'];
        $message = $results['message'];
        $version = "2.8.11";
    ...
    // UPGRADE TO V2.8.12
    if ($common->getSetting("version") == "2.8.11" && $success) {
        $json = file_get_contents("http://localhost/install/upgrade-v2.8.12.php");
        $results = json_decode($json, TRUE);
        $success = $results['success'];
        $message = $results['message'];
        $version = "2.8.12";
