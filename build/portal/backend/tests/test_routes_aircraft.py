...
# GET /api/adsb/aircraft/{icao}

def test_get_aircraft_by_icao_200(client):
    response = client.get('/api/adsb/aircraft/icao01')
...
def test_get_aircraft_by_icao_404(client):
    response = client.get('/api/adsb/aircraft/icao00')
    assert response.status_code == 404

# GET /api/adsb/aircraft/{icao}/positions

def test_get_aircraft_positions_200(client):
    response = client.get('/api/adsb/aircraft/icao05/positions')
    assert response.status_code == 200
...
def test_get_aircraft_404(client):
    response = client.get('/api/adsb/aircraft/icao00/positions')
    assert response.status_code == 404

# GET /api/adsb/aircraft

def test_get_all_aircraft_200(client):
    response = client.get('/api/adsb/aircraft')
    assert response.status_code == 200
    assert response.json['offset'] == 0
...
def test_get_all_aircraft_200_offset(client):
    response = client.get('/api/adsb/aircraft?offset=2')
    assert response.status_code == 200
...
def test_get_all_aircraft_200_limit(client):
    response = client.get('/api/adsb/aircraft?limit=2')
    assert response.status_code == 200
    assert response.json['offset'] == 0
...
def test_get_all_aircraft_200_offset_and_limit(client):
    response = client.get('/api/adsb/aircraft?offset=1&limit=2')
    assert response.status_code == 200
...
def test_get_aircraft_400_offset_less_than_0(client):
    response = client.get('/api/adsb/aircraft?offset=-1')
    assert response.status_code == 400

def test_get_aircraft_400_limit_less_than_0(client):
    response = client.get('/api/adsb/aircraft?limit=-1')
    assert response.status_code == 400

def test_get_aircraft_400_limit_greater_than_100(client):
    response = client.get('/api/adsb/aircraft?limit=101')
    assert response.status_code == 400

# GET /api/adsb/aircraft/count

def test_get_aircraft_count(client):
    response = client.get('/api/adsb/aircraft/count')
    assert response.status_code == 200
    assert response.json["aircraft"] == 5

# GET /api/adsb/aircraft/{icao}/positions — aircraft with no positions

def test_get_aircraft_positions_empty(client):
    # icao04 exists but has no positions in the test data
    response = client.get('/api/adsb/aircraft/icao04/positions')
    assert response.status_code == 200
    assert response.json['count'] == 0
    assert response.json['positions'] == []

# Pagination boundary — offset beyond total count returns empty list

def test_get_aircraft_positions_offset_beyond_count(client):
    response = client.get('/api/adsb/aircraft/icao01/positions?offset=100')
    assert response.status_code == 200
    assert response.json['count'] == 0
    assert response.json['positions'] == []
...
def test_get_aircraft_cors_headers(client):
    response = client.get('/api/adsb/aircraft/icao01')
    assert 'Access-Control-Allow-Origin' in response.headers
    assert response.headers['Access-Control-Allow-Origin'] == '*'

def test_get_aircraft_positions_cors_headers(client):
    response = client.get('/api/adsb/aircraft/icao01/positions')
    assert 'Access-Control-Allow-Origin' in response.headers
    assert response.headers['Access-Control-Allow-Origin'] == '*'

# Response structure checks

def test_get_aircraft_response_structure(client):
    response = client.get('/api/adsb/aircraft/icao01')
    assert response.status_code == 200
    for field in ('id', 'icao', 'first_seen', 'last_seen'):
        assert field in response.json

def test_get_aircraft_positions_response_structure(client):
    response = client.get('/api/adsb/aircraft/icao01/positions')
    assert response.status_code == 200
    for field in ('offset', 'limit', 'count', 'positions'):
        assert field in response.json
    pos = response.json['positions'][0]
    for field in ('id', 'flight', 'aircraft', 'time', 'message', 'squawk',
                  'latitude', 'longitude', 'track', 'altitude', 'vertical_rate', 'speed'):
        assert field in pos

# Unsupported HTTP methods → 405

def test_aircraft_post_method_not_allowed(client):
    response = client.post('/api/adsb/aircraft/icao01')
    assert response.status_code == 405

def test_aircraft_positions_post_method_not_allowed(client):
    response = client.post('/api/adsb/aircraft/icao01/positions')
    assert response.status_code == 405
