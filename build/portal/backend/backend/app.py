...
@app.route('/api/v1/blog/post', methods=['POST'])
def post_blog_post():
...
@app.route('/api/v1/blog/post/<int:id>', methods=['DELETE'])
...
@app.route('/api/v1/blog/post/<int:id>', methods=['GET'])
...
@app.route('/api/v1/blog/post/<int:id>', methods=['PUT'])
...
@app.route('/api/v1/blog/posts', methods=['GET'], defaults={'offset': 0, 'limit': 100})
...
@app.route('/api/v1/flight/<id>', methods=['GET'])
def get_flight(id):
    data=[]
    ...
    try:
        connection = create_connetion()
        cursor=connection.cursor()
        cursor.execute("SELECT * FROM flights WHERE flight = %s", (id))
        columns=[x[0] for x in cursor.description]
        result=cursor.fetchall()
        for result in result:
            data.append(dict(zip(columns,result)))
    except Exception as ex:
        logging.error(f"Error encountered while trying to get flight id {id}", exc_info=ex)
        abort(500, description="Internal Server Error")
    finally:
        connection.close()
...
@app.route('/api/v1/flight/count', methods=['GET'])
...
@app.route('/api/v1/flights', methods=['GET'], defaults={'offset': 0, 'limit': 100})
