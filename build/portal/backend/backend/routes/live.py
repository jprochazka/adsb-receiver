...
_LAST_KNOWN_LIVE_AIRCRAFT_PAYLOAD = None
...
class LiveAircraftResource(Resource):
    ...
    def get(self):
        ...
        if dump1090_data is None and dump978_data is None:
            fallback_payload = _LAST_KNOWN_LIVE_AIRCRAFT_PAYLOAD
            if fallback_payload is not None:
                payload = {
                    **fallback_payload,
                    'source_status': 'stale-cache',
                    'detail': '; '.join(errors) if errors else 'No decoder data available',
                }
                elapsed_ms = (time.perf_counter() - request_started) * 1000
                logging.warning('live_aircraft stale-cache aircraft=%d elapsed_ms=%.2f detail=%s', len(payload.get('aircraft', [])), elapsed_ms, payload.get('detail'))
                return payload, 200
            ...
            payload = {
                ...
                'messages': 0,
                'aircraft': [],
                'classification_stats': _classification_stats([]),
                'source_status': 'offline',
                'detail': '; '.join(errors) if errors else 'No decoder data available',
            }
            elapsed_ms = (time.perf_counter() - request_started) * 1000
            logging.warning('live_aircraft offline aircraft=0 elapsed_ms=%.2f detail=%s', elapsed_ms, payload['detail'])
            return payload, 200
